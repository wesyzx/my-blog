      ...
      >
        <div className="max-w-[1100px] mx-auto px-6 h-[52px] flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item) => {
              const isActive = pathname === item.href || (item.hasDropdown && pathname === '/' && item.href === '/')
              return item.hasDropdown ? (
                <div key={item.label} className="relative group">
                  ...
                  <div className="absolute left-0 top-full pt-2 hidden group-hover:block animate-fade-up">
                    <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-[var(--radius-md)] p-2 min-w-[120px] shadow-xl">
